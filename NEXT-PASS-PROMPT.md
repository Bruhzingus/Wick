...
- 54 deterministic pure-rule tests with a Studio-only runner
- feel/audio plumbing and a visible desktop/touch control hotbar
- loot, free tool charges, session-local remains, and seven depth-weighted threats
...
- a larger/slower/sparser 10-floor pacing surface and result-screen death/restart handling

Important limitations: Studio uses ProfileStore Mock and local expedition startup; audio IDs are
empty; the lobby is not full matchmaking; rejoin recovery and global remains are deferred.
...
5. approved audio IDs and a focused sound/feel pass.
