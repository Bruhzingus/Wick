...
function StoneWardenModel.buildDormantPile(spawnCFrame)
    ...
    for i = 1, 25 do
        ...
        local offset = CFrame.new(math.random(-6, 6), math.random(-2, 4), math.random(-6, 6)) * CFrame.Angles(math.random(), math.random(), math.random())
        local rock = createSlab(model, Vector3.new(scale, scale*1.3, scale), spawnCFrame * offset, math.random(1,3), math.random(1,3))
...
function StoneWardenModel.buildActiveWarden(spawnCFrame)
    ...
    humanoid.WalkSpeed = 8 -- Half of default 16
    ...
    local yaw = 0
    local torsoDims = {
        {w = 4.0, h = 1.5, d = 3.0}, {w = 5.0, h = 1.8, d = 3.5},
        {w = 6.0, h = 2.0, d = 4.0}, {w = 7.0, h = 1.5, d = 3.0},
        {w = 3.0, h = 1.5, d = 2.5}
    }
    ...
    local function buildLeg(side)
        ...
        weldVisual(hitbox, thigh); weldVisual(hitbox, knee); weldVisual(hitbox, calf); weldVisual(hitbox, foot)
    end
    ...
    local function buildArm(side)
        ...
        weldVisual(hitbox, upperArm); weldVisual(hitbox, elbow); weldVisual(hitbox, forearm); weldVisual(hitbox, hand)
    end
