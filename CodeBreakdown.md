...
| How much of a cave the descent WAITS for, versus how much arrives afterwards | `server/RunOrchestrator.luau` (`ensureLookahead` / `buildOnePendingFloor`) | `Config/RunSettings.floorLookahead`; floor one is the only synchronous build and the only thing standing between a party and a body, so everything else is queued and drained one floor per frame. `checkDescent` still builds on demand, which is what makes deferral safe. Timings land in the `floor_spawn_audit` (`planMs`/`carveMs`/`populateMs`) and `run_started` (`blockingFloorMs`) telemetry lines, and `replicatedfirst/WickLoadingScreen.client.luau` prints which gate held the screen |
...
- `Diagnostics`, `DevTestRoom`: the observation-only developer overlay and the separate physical enemy
  ...
  static entity-guide cards, fixed console actions, virtual depth, spawn cap, free charges and utility
  timings; all lab mutations remain server-owned.
