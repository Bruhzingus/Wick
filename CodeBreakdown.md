...
| Ashamed Lurker placement, trip lane, grab, stare-to-clear, relocation, or procedural state poses | `Config/AshamedLurker.luau` / `Logic/AshamedLurkerRules.luau` / `server/AshamedLurkerService.luau` / `NewModelsAndObjects/AshamedLurker.luau` | `Logic/FloorPlanner` (`planLurkers`), `server/FloorBuilder` (proxy + `LurkerSite`), `shared/AshamedLurkerVisualProtocol`, `client/AshamedLurkerController`, `Net/Remotes.LurkerGaze`, `Config/Security.remoteRateLimits.LurkerGaze`, `WaxService.drainExternal`, `Types/World.LurkerPlacement`, `Tests/AshamedLurkerRulesTests`. `triggeredStateSeconds` is grab delay + full recovery; `visualLungeStuds` separately clamps presentation inside the trapped half. A grab survived fires `Config/Feel.tutorialHints.AshamedLurker` once ever per player (`Interfaces/Persistence.claimFirstSighting`, bypassing `tutorialHints.maxDepth` since the Lurker cannot appear before floor 4); `drainExternal`'s trailing `sourceId` is `"AshamedLurker"` either way, for `Config/Feel.deathHints` if the grab kills |
...
| VoidFly patrol path, harassment commitment, chase speed, or what noise does to one | `Config/Threats.definitions.VoidFly.ambush` + `.hearing` | `Logic/ThreatBrain` (`patrolTarget`, `thinkAmbusher`, the `step` leash and pursuit speed), `Logic/ThreatRules.roofProfile` (both roof-bound rows share the patrol knobs), `server/ThreatService` (`ambushNeedsImmediateReaction` uses the pursuit reach so a flare interrupts a dive off-patch), `Types/Threat.AmbushProfile`/`ThreatState.pursuitUntil`, `Tests/ThreatBrainTests`. `pursuitSpeedMultiplier` must stay between `Config/Movement.walkSpeed` and `.runSpeed` or the creature's counterplay changes category |
...
| Gas Lantern preview/payout, individual or unanimous group extraction, five-second free look, shared ignition, and the first-ignition enemy purge | `server/BrazierService.luau` | `Logic/ExtractionValue`, `Logic/CargoRules`, `Config/Brazier` (validation/hold), `Config/Lantern.network` (one lamp per room, local point glow plus directional amber coverage, real-surface clearance, ordered catch, five-second window), `Config/Extraction`, `NewModelsAndObjects/Lantern` (`mountToWall` owns the wall-relative SpotLight), `server/FloorBuilder` (deterministic per-room wall selection, doorway-safe fallback position, sampled-ground mounting height, base/middle/top Terrain surface probes, dressing exclusion, then distance-ordered lamp runtime), `server/ExtractionService.unresolvedPartyMembers` (authoritative ready denominator), `server/CharacterService.freeze`/`.face`, `server/ThreatService.clearDepth`, `server/AshamedLurkerService.despawnDepth`, `server/StoneWardenService.clearDepth`, `Interfaces/Persistence`, `client/LanternPresentation`, `client/ResultsText`, `RunEvent("lanternLit")`. The completion room's interactive Gas Lantern is its only lamp; every other generated room deterministically gets exactly one network lamp. Lamps mount just proud of the nearest real Terrain wall surface without carving an indent. The individual prompt always ends only its user: if another runner remains, the result card is immediate; the last unresolved runner gets normal first-person look for five seconds. The separate GROUP EXTRACT prompt requires every unresolved runner to opt in at the same depth and validated radius, then starts their independent payouts on one server tick and reveals their cards together. Readiness is dropped on distance/state/floor loss, so proximity alone cannot force extraction. On first ignition, that depth's generic threats, Ashamed Lurkers, and Stone Warden are removed; deeper enemies and non-enemy hazards remain. There are no flying orb effects and no scripted camera. This is a beat IN PLACE — never a return-to-surface sequence. Death deliberately has no equivalent (see `server/DeathService`) |
